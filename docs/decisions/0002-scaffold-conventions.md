...
- PR6（プロンプトと要求の組み立て）後：`pnpm check` はテストファイル 23 件・テスト 529 件が WSL で通過（Windows は CI で確認）（2026-09-08、レビュー対応後の値）。この作業環境からは LM Studio に到達できず、`pnpm test:llm` は未実行。`SHUTEN_LM_STUDIO_URL` を設定していない状態でテストファイル 2 件・テスト 11 件が skip されることだけ確認した。
