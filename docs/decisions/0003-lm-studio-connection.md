...
- **思考の無効化はトップレベルの `reasoning_effort` で試す。** gemma では `reasoning_effort: "none"` で思考が
  完全に止まった（`reasoning: { effort }` の入れ子形式や `chat_template_kwargs` は効かない）。
  qwen ではトップレベル形式が未検証。思考の有無は生成設定として実行記録に残し、評価で比較する。
- **JSON スキーマの `description` はモデルに渡らない。** LM Studio はスキーマを文法制約にだけ使う。
  各項目の意味（引用、前後の引用、修正案、理由）はプロンプト本文に書く。
- **比較実験では `seed` を固定する。** temperature 0.7 でも同じ seed で同一出力になった。
- 切断による停止、`reasoning_content` の分離、`json_schema`、seed は 2 モデルで同じ挙動であり、
  LM Studio 側の挙動として扱ってよい。
...
| 項目 | 状態 |
| --- | --- |
| モデル ID | 未決。qwen（86 秒、3/3 検出）と gemma（24 秒、2/3 検出、思考なしで 5 秒）を 1 原稿で観察。評価原稿で比較する |
| プロンプト、生成パラメーター | 未決。`max_tokens` の初期値だけ 16,000 |
| タイムアウト | 未決。qwen で 1 要求 2〜3 分（906 字で 86 秒）、gemma で 30 秒前後を前提に評価時に決める |
| 分割長と文脈長の実測調整、評価原稿 | 未決 |

## 品質の初期観察（評価ではない）

反復のない 906 字の創作文に埋め込んだ誤り 3 件（重複、助詞抜け、誤変換）に対し、qwen は 3 件すべてを検出、
