...
WSL2 の既定（NAT）構成から Windows 側の LM Studio へ接続するには、`localhost` ではなく Windows ホストの IP を使う。
デフォルトゲートウェイがそれに当たる。`networkingMode=mirrored` の場合は `127.0.0.1` で届く。
...
- 修正案は引用した文全体に対応し、その中の必要な文字だけを直している。これは「最小限の変更」と整合する。
  プロンプトでは「suggestion は quote 全体に対応し、変更内容を最小限にする」と指示する。引用を短くする改善とは別の話。
