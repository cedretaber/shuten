...
> この 2 項目（`/settings` の直リンクとリダイレクト）は **PR11c では実施していない**。
> SPA フォールバックは `app.get("*")` でパスに依存せず、リダイレクトはブラウザ内の React Router で
> 完結するため、PR11 の実機確認と Windows CI で担保できると判断した（PR11c 計画書の
> 「Windows での再確認」節）。次にこの手順を通すときに埋めること。

- [ ] `http://127.0.0.1:3000/runs/does-not-exist` を直接開くと、サーバーの 404 ではなく
      **画面内の「その実行はありません」**が表示される
- [ ] 「設定」画面を開くと（起動時に自動で 1 回、接続確認が走る）、ヘッダーに
