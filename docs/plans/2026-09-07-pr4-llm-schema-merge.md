...
統合結果・抑制結果の保存と再閲覧（PR8/PR12）、失敗観点の再試行時に既存の統合結果へ参照を追加する処理（PR9。
...
3. **JSON Schema は検証済みの形に合わせる。** `docs/decisions/0003` で疎通を確認した要求は、各オブジェクトに
   `additionalProperties: false`、全キーを `required`、`$schema` なし、整数の `maximum` なし。ただし検証した
   スキーマの項目は文字列だけで、本 PR が加える `minimum`、`minLength`、`type: ["string", "null"]`、`enum` は
   LM Studio で未確認。生成した実スキーマでの疎通試験を PR6 までに 1 回行う（モデルの交換は不要）。それまで
   「スキーマ全体を確認済み」とは言わない。生成後に `$schema` を
...
- **統合時の `verdict`（決定 8）。** 仕様 6.4 は統合後の暫定判定を規定していない。本計画は「全候補が
  ...
  「1 件でも `likely-error` なら `likely-error`」（2 観点が同じ修正で一致した事実を重く見る）。再確認が正常終了すれば
  その結果が最終判定になるが、再確認なしの比較実験と、再確認の失敗・無効化時（未検証の初回判定として表示。仕様 5.4）には
  この値がそのまま見える。外部レビューで前者を採用した。
...
- **前後への挿入だけの修正案（決定 10）。** 代替案は字義どおり「置換文字列が空でなく登録語と異なる」だけで判定し、
  ...
  カタカナの固有名詞で起こりやすいが、「範囲外に及ぶ変更」「迷う場合は残す」に合うとして外部レビューで本計画を採用した。
  また本決定が防ぐのは純粋な挿入だけで、
...
- **空の引用の拒否。** `quote` の `minLength: 1` は、構造化出力では文法で空引用を防ぎ、非対応モデルの経路では空引用を
  ...
  そのまま拒否する。位置を持てず利用できない候補なので限定再要求へ回す方が一貫する、として外部レビューで採用された。
