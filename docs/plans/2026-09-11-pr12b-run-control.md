...
- 実 LLM を動かした「実行中」の表示（進捗の更新、遅延の通知、停止の効き方、`run-settled` の後に
  `/events` が増え続けないこと）は未確認。この環境には LM Studio が無く、実行が画面を開く前に
  終端状態へ落ちるため確認できなかった（Task 9 報告）。
- Windows 未確認。
- 指摘一覧が入った状態での右側（`.sideColumn`）の独立スクロールと、「指摘へ移動」したときの
  `scrollIntoView` の挙動が実ブラウザ未確認。LM Studio が無く生成が走らないため、Task 9 の確認では
  指摘 0 件の実行しか作れず、`.bodyColumn` 側の内部スクロールが機能することは実測したが、
  `.sideColumn` 側で一覧が実際に長くなったときの挙動と `scrollIntoView` が `.bodyColumn` 内部で
  効くことは未確認のまま残る。
- `GET /api/runs/:id/findings` の 3N+1 は引き続き PR12c の担当。本 PR の合流（決定 3）は緩和であって
  解消ではない。
  → **2026-09-12 の PR12c で解消**（`docs/plans/2026-09-11-pr12c-findings-batch.md`）。
  実行 1 件ぶんのバッチ取得にまとめ、問い合わせは指摘の件数によらず 5 本になった。
...
- 初回読み込みで `GET /api/runs/:id/units` の取得に失敗すると、結果画面全体が表示されない。
  ...
  かったため、その頃は読めていた本文・指摘一覧が、`/units` 1 本の失敗で読めなくなるという後退が
  生じている。
- 2 カラムの `height`（決定 15。`calc(100vh - 3.5rem - (2 * var(--space-lg)))`）に `min-height` の
  逃げが無く、極端に短いビューポートでは計算値が負になりうる。狭い幅で縦積みにするレイアウトの
  切り替えも入れていない（`.layout` は常に横 2 カラム）。
- 操作の失敗の案内（409 の定型文「…最新の状態を取得しました。」）が、直後の取り直しの成功で消える
  窓がごく狭い条件で残っている。`clearControlFailureRef` は手動の取り直し 1 本の終了で消費するため、
  「最新の状態を取得」ボタンを押した取り直しが走っている最中に別の操作で 409 が返ると、その取り直し
  の成功で案内が消えてしまう。踏むには「更新ボタンを押した直後に操作の失敗が返る」必要があり、
  発生条件が狭いため本 PR では塞がず見送った（Task 8 報告）。
