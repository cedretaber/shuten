...
- 保存中（`JudgmentControl` の「保存」を押して応答待ちの間）に別の指摘へ切り替えると、`key={finding.id}`
  で操作子ごと作り直されるため、直前の保存が失敗してもエラー表示がアンマウントで消え、気づけない
  （成功は取りこぼさない。失敗だけ何も出ない）。最終レビュー Minor 4 で指摘されたが、この PR では
  直さない（既知の制限として記録するに留める）。
