...
作成日：2026-09-08
状態：計画（レビュー前）
仕様：`docs/spec/mvp-spec.md`（v0.8）6 全体、7、10、11 節（3・4・19・21 項）、13
...
| `packages/cli/` | 新規パッケージ `@shuten/cli`。`src/main.ts`、`bin/shuten-eval.ts` |
| `server/package.json` | `exports` を追加（`packages/cli` から `@shuten/server/run/pipeline.ts` を読むため） |
...
/** 1 単位（1 検査対象 × 1 観点、または 1 指摘の再確認）の処理状態。 */
export type UnitStatus = "pending" | "done" | "failed";

/** 失敗の記録。reason は shared の FailureReason をそのまま使う。 */
...
export type RecheckResult =
  ...
  | { readonly status: "failed"; readonly attempts: number; readonly failure: UnitFailure }
  | {
...
export interface RunTotals {
  ...
  readonly unlocated: { readonly notFound: number; readonly ambiguous: number; readonly outsideTarget: number };
  readonly findings: number;
  readonly suppressed: number;
  readonly rechecks: { readonly done: number; readonly failed: number; readonly skipped: number };
  readonly elapsedMs: number;
}
...
export interface PipelineArgs {
  ...
  readonly mode: PipelineMode;
  readonly perspectives: readonly Perspective[];
  /** 改行区切りの生の文字列。分割・trim はパイプラインが行う。 */
...
export type ExecOutcome<T> =
  | { readonly ok: true; readonly value: T; readonly attempts: number;
      readonly usage: Usage | null; readonly elapsedMs: number }
  | { readonly ok: false; readonly attempts: number; readonly failure: UnitFailure;
      /** 非 null なら実行全体を止める。 */
      readonly halt: StopReason | null };

export interface Executor {
  /** 直前に ensureLoaded を呼び、chat を送り、parse に通す。同時実行は 1。 */
  execute<T>(
    request: ChatRequest,
    parse: (result: ChatResult) => T,
    timeoutMs: number,
  ): Promise<ExecOutcome<T>>;
  /** 最初に成功した ensureLoaded の結果。 */
...
検査対象をまたいだ統合は行わない。処理は検査対象の `index` 昇順。

各検査対象での順序：

1. `buildCheckInput`（`full-text` では後述の全文入力）
...
- `pending`：生成要求を 1 度も送っていない。モデル未ロードや実行の途中終了で残った単位。
  仕様 7 節の「当該処理を失敗にせず未完了のまま残す」に対応する。
- `failed`：送ったが失敗した。`attempts` と `UnitFailure` を持つ。
- `done`：応答をスキーマまで通した。

`failed` と `pending` を混ぜない。JSON を見て「送っていない」と「送って失敗した」が区別できること。
...
| `truncated` | 1 回 | 同上 |
| `connection` | 1 回 | 2 回目も `connection` なら実行を `stopped`（`connection-lost`） |
| `model-not-loaded` | しない | 実行を `stopped`（`model-not-loaded`）。当該単位は `pending` のまま |
| `timeout` | しない | 実行を `stopped`（`recovery-needed`）。当該単位は `failed` |
| `aborted` | しない | 実行を `stopped`（`aborted`） |
| `input-too-long` | しない | 実行を `stopped`（`settings`） |

理由：

- `timeout` の後は、生成が LM Studio 側で走り続けているかを本 PR では確認できない。
  仕様 7 節・8.2 節が求める「生成終了を確認できるまで後続を送らない」を守るため、
  後続の要求を送らずに終了する。上限付き待機による復旧は PR9。
- `connection` を 1 単位ずつ失敗させると、LM Studio が落ちている場合に残り全単位で
  同じ待ち時間を消費する。2 回連続なら実行を止める。
- `model-not-loaded` は仕様 7 節が「案内して待機」と定めているので、失敗にせず `pending` で残す。

### 決定 6：`stop` / `length` 以外の `finish_reason`（PR6 からの持ち越し）

PR5 のクライアントは `length` を `truncated` の例外にし、それ以外の値は `ChatResult.finishReason`
に載せて返す。パイプラインは `finishReason !== "stop"` の応答を成功にせず、`truncated`
（`UnitFailure.finishReason` に実際の値を入れる）として扱う。
...
`splitAllowedWords(raw: string): string[]` を `server/src/run/allowed-words.ts` に置く。
`/\r\n|\r|\n/` で分割し、各行を `String.prototype.trim` にかけ、空文字を除き、完全一致の重複を
除いて出現順に返す（`shared` の `findSuppression` は登録語を加工しない、という PR4 の分担を守る）。
重複除去は仕様に明記がないが、`findSuppression` の結果を変えず、実行条件の記録が読みやすくなるので行う。
...
`maxInputGraphemes` の検査は行い、超えたら `input-too-long` で止める（縮めない）。
...
`createId` の既定は `c1`, `c2`, … の連番（候補用）と `f1`, `f2`, …（統合後）。
...
- そうでなく、`failed` の単位が 1 つでもあれば `partially-failed`（仕様 6.4）。
- どちらでもなければ `completed`。

再確認の失敗も `partially-failed` に数える。抑制・`disabled` は失敗ではない。
...
- 原稿はバイト列で読み、`ingestUtf8Bytes` を通す（BOM と CRLF の扱いを server と揃える）。
- 許容語ファイルも同じ経路で読み、`splitAllowedWords` に渡す。
- 接続先と API キーは server と同じ環境変数（`SHUTEN_LM_STUDIO_URL`、`SHUTEN_LM_STUDIO_API_KEY`）
  から読む。CLI の引数では受け取らない（履歴に API キーを残さないため）。
...
`@shuten/server/run/pipeline.ts` を解決できない。`shared` と同じ形（`"./*": "./src/*"`）を足す。
`packages/cli` は新しいワークスペースなので、`typecheck` スクリプトと `tsconfig.json`、
`vitest.config.ts`（`name: "cli"`）を置く（ルートの `vitest.config.ts` は `packages/*` を拾う）。

## 状態モデル（一覧）

| 対象 | 状態 | 意味 |
| --- | --- | --- |
| 検査単位（対象 × 観点） | `pending` | 未送信。未ロードや途中終了で残った |
| | `done` | 応答をスキーマまで通した |
| | `failed` | 送ったが失敗（`attempts` と理由を持つ） |
...
本 PR では暫定値を CLI の既定として置き、計画・PR 本文・結果 JSON に「暫定」と明記する。
...
| `checkTimeoutMs` | 120000 | PR6 の実測（短い合成原稿で 2〜3 秒）に対して十分な余裕。実原稿の長さで測り直す |
| `recheckTimeoutMs` | 120000 | 同上 |
...
2. `timeout` で実行全体を止める（決定 5）。仕様 7 節の「生成終了を確認できるまで後続を送らない」を
   PR7 の範囲で守る最も安全な解釈だが、1 単位のタイムアウトで全体が止まるため、実原稿の試運転で
   タイムアウト値が短すぎると何度もやり直すことになる。
3. `truncated` の再試行を 1 回行う（決定 5）。`temperature: 0` では同じ結果になる可能性が高く、
   時間の無駄になりうる。仕様 7 節の「自動再試行は各処理 1 回」を素直に適用した。
4. `connection` の 2 回連続で実行を止める（決定 5）。仕様に明記はない。
5. 許容語の重複除去（決定 7）。仕様は分割規則しか定めていない。
6. `full-text` 方式が構造化プロンプトのままである点（決定 8）。仕様 10 節の「現在の全文チャット方式」
   との厳密な比較にはならない。自由形式のプロンプトを別に用意すべきかは PR13 の設計で決めたい。
7. 実原稿での試運転には作者が利用を認めた原稿が要る（仕様 10 節）。本 PR では合成の長文で
   パイプラインを通し、実原稿での試運転はユーザーの原稿が用意できた時点で行う。
   合成原稿での試運転だけで PR を締めてよいかを確認したい。

## PR8・PR9・PR13 への持ち越し

- 永続化、実行 ID による `mergeKey` の名前空間分離、失敗観点の再試行での参照追加（PR8・PR9）。
- 生成終了の確認と上限付き待機（`recovery-waiting`）、複数実行の単一キュー（PR9）。
- 評価指標の集計（検出率、誤検出、位置特定失敗率、診断候補の正誤、抑制の適否）と、
  段落マーカーの引用への混入率・検査対象の終端をまたぐ引用の集計（PR13）。
  本 PR は結果 JSON に生の `LlmFinding` と `LocateResult` を残すところまでとし、
  集計ロジックをパイプラインに入れない。
- `<think>` の分離（実測で必要になったモデルが出たときに、決定記録 0003 への追記と同時に）。
- `before` / `after` の長さの調整（PR6 からの持ち越し。試運転の観測を材料にする）。

## テスト

### U：`allowed-words.ts` と `result.ts`（単体）
...
| E4 | `truncated`（`length`）も同様に 1 回再試行し、`maxTokens` を変えずに送る |
| E5 | `connection` が 2 回続くと `halt: "connection-lost"` |
| E6 | `model-not-loaded` は再試行せず `halt: "model-not-loaded"`、`attempts: 0` |
| E7 | `timeout` は再試行せず `halt: "recovery-needed"` |
| E8 | 呼び出し元の `signal` が中断されると `halt: "aborted"` |
| E9 | 最初の `ensureLoaded` の `ModelInfo` が `modelInfo` に残る |
| E10 | 失敗のメッセージと `raw` に API キーが現れない |
...
| P5 | 存在しない引用は `unlocated`（`not-found`）に入り、再確認要求が送られない |
| P6 | 参考文脈内から始まる引用は `outside-target` として `unlocated` に入る |
| P7 | 許容語に一致する表記訂正は `suppression` が付き、再確認要求が送られない（呼び出し回数で検証） |
| P8 | `mode: "split"` では再確認要求が 0 件、各 `recheck.status` が `disabled` |
| P9 | 1 観点だけ `malformed` が続くと、もう一方の観点の指摘で統合が進み `partially-failed` |
| P10 | 全観点が失敗した対象があっても、他の対象の結果は残り、指摘ゼロと区別できる |
| P11 | `timeout` の後に `chat` が 1 度も呼ばれず、残りの単位が `pending`、`stop.reason` が `recovery-needed` |
| P12 | k 番目の `ensureLoaded` が `model-not-loaded` のとき、それ以前は `done`、k 番目以降は `pending`、`stopped` |
| P13 | `signal` を途中で中断すると `stopped`（`aborted`）で、以後の要求が送られない |
| P14 | `InputTooLongError`（`maxInputGraphemes` を小さくする）で `stopped`（`settings`）、本文は縮まない |
| P15 | `InvalidChunkSettingsError` も `stopped`（`settings`） |
| P16 | `mode: "full-text"` で観点ごとに要求が 1 件だけ、`targets` が 1 件 |
| P17 | 同じ入力・同じモック応答で 2 回実行すると ID と結果 JSON が一致する |
| P18 | `conditions` に接続先 URL と API キーが現れない |
| P19 | `conditions.versions` が `PROMPT_VERSION` などの実際の値を持つ |
| P20 | `onEvent` が例外を投げても実行が続き、結果が変わらない |
| P21 | `inputGraphemes` が要求に載せた本文の書記素数と一致する（CRLF・絵文字を含む本文で） |
...
| C4 | `--mode full-text` と `--recheck-context-graphemes` の同時指定を拒否しない（無視して記録） |
| C5 | BOM 付き CRLF の原稿ファイルを読み、本文の位置が server と一致する |
| C6 | 許容語ファイルの CRLF 分割 |
| C7 | 結果 JSON が `PipelineResult` の形で、`JSON.parse` して再検証できる |
...
| # | 内容 |
| --- | --- |
| I1 | 合成の長文（複数の検査対象に分かれる長さ）で `mode: "split-recheck"` が通り、`completed` |
| I2 | `mode: "full-text"` が通る |
| I3 | 各要求の `inputGraphemes` と `usage.promptTokens` を記録する（assert なし。換算係数の材料） |
...
2. 本計画をエージェント 2 体で自己レビューし、指摘を反映してユーザーへ提出する。
3. 実装は SDD（`superpowers:subagent-driven-development`）で行う。ロードマップの担当欄は
   ...
   2. `executor.ts`（E）
   3. `pipeline.ts`（P）
   4. `packages/cli` と `server` の `exports`（C）
   5. PR5 持ち越しテスト（L）
