...
| `checkTimeoutMs` | 300000 | 決定記録 0003「思考ありの qwen で 1 要求 2〜3 分、思考なしなら 10 秒前後」。決定 5 では 1 件のタイムアウトが実行全体を止めるので長めに取る。通常運用は思考なし（決定記録 0003 の 2026-09-09 の節）で 1 要求 10 秒前後なので大きく余るが、比較のために思考ありで回すときに備えてこの値を既定にしている。なお試運転当初は Node の `fetch`（undici）の `headersTimeout` 既定 300 秒が実装側の上限になっており、300 秒より長い値を渡しても効かなかった（修正済み。決定記録 0003 の 2026-09-08 の節） |
...
- **`createLmStudioClient` が作る `undici.Agent` を解放する手段がない**（PR10）。`LmStudioClient` に
  `close()` / `dispose()` がなく、CLI はプロセスあたり 1 クライアントなので現状は実害ないが、
  要求ごとにクライアントを作る形にすると keep-alive ソケットが溜まる。PR10 で HTTP API 化する際に必要になる。
