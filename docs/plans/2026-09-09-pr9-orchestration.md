...
- **（解消済み）`queue` を渡した経路の二重の直列化。** PR9a の当初の実装は、`run/executor.ts` が自前の
  `tail` で直列化したうえでジョブ全体をキューへ投入していた。これは無駄な二段構えであるだけでなく、
  投入順が `execute` の呼び出し順とずれる（別々の executor に `A1 → A2 → B1` の順で呼ぶと、`A2` は `A1` の
  完了を待ってから投入されるのに対し `B1` は即座に投入されるため、実行順が `A1 → B1 → A2` になる）という
  観測可能な問題を伴っていた。PR #14 のレビューで指摘され、**`queue` が渡されたときは `tail` を経由せず
  ただちに投入する形に直した**（`run/queue.ts` の Q4b が呼び出し順と実行順の一致を固定している）。
  `tail` はキューを渡さない経路（`runPipeline` / CLI）専用として残っている。PR9b の作業項目ではない。
...
- **停止操作で打ち切った単位の `pending_note` はまだ決定 20 の文言になっていない。** PR9a は
  `run/units.ts` の `pendingNote` で、ハード上限超過（`recoveryConfirmMs > 0` かつ `timeout`）の
  ケースだけ決定 20 の文言（「応答が上限内に届かなかった。生成終了は未確認」）を入れる。停止操作
  （`aborted`）で打ち切った単位には従来どおり `failure.message` が入る。決定 20 が定める
  「停止操作により打ち切った。生成終了は未確認」を入れる経路は、決定 6 の 4（上限まで待ってから
  abort する制御）を実装する PR9b の `run/recovery.ts` 側にある。PR9b でその制御を書くときに、
  停止経路の `pending_note` も決定 20 の文言に合わせること。
