...
ロードマップはこれを「着手時に確認する」提案としていたので、時系列順に並ぶ ID
（UUIDv7・ULID）と比べたうえで v4 のままにする。

- `crypto.randomUUIDv7()` は Node 24.20.0 に存在する（依存の追加は不要）。ただし RFC 9562 は
  ミリ秒内の単調増加を必須にしておらず、同一ミリ秒に 50 件生成して並べたところ
  文字列順と生成順が一致しなかった。ULID も同じで、保証するには状態を持つ monotonic 生成器が要る。
- 仮に単調な ID を使えても、候補の順序が「ID を発行した順」という実装の副作用に依存する。
  保存処理の分割や再試行で崩れる。順序は業務データとして `candidate_index` に持つ（決定 19）。
- 時系列 ID による SQLite の索引局所性は、1 実行あたり数千行の規模では効かない。

ID は不透明で一意なだけの値に留め、時刻は `created_at`、順序は `candidate_index` が担う。
