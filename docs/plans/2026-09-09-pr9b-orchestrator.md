...
すべて超過していれば**生成要求を 1 件も送らずに**）、`clearStopState` が停止理由まで消す。
...
**一部の対象だけが上限を超えた実行は、`pending` の単位を残したまま進められなくなる。**
その実行は `stopped`（`settings`）でありながら未検査の対象を持つが、決定 36 のこの規定により
`resumeRun` は拒否し、`retryFailedUnits` も「戻せる `failed` 単位が無い」（`input-too-long` は
除外対象）ので拒否する。分割設定は実行ごとに固定なので、上限を超えた対象はその実行では
永久に検査できない。**設定を見直して新しい実行を開始する**のが復旧手段である。
未検査の対象だけを進めて `partially-failed` で終える案は採らない。1 度も検査していない対象を
含む実行を「完了扱い」に近づけることになり、この規定の目的（設定エラーを完了と誤表示しない）
と衝突するためである。
...
ただし `recovery-waiting` の実行を版の食い違いで拒否するときは、**拒否の前に
`deps.recoveryGate.unblock(runId)` を呼ぶ**。決定 39 が言うとおり、ゲートを開ける行為の意味は
「利用者が LM Studio 側の生成終了を確認した」であって「この実行を続ける」ではない。再開を
指示した時点で確認は済んでおり、その実行を続けられるかどうかとは独立している。これを
怠ると、版が変わった `recovery-waiting` の実行が**プロセス全体の送信ゲートを永久に閉じたまま
にする**（`reconcileOnStartup` が起動のたびに閉じ直すので再起動でも解けず、案内どおり新しい
実行を始めても `recovery-blocked` で止まる）。

なお拒否理由の判別子（`RunLaunchResult` にどの条件で弾いたかを載せる）は本 PR に入れない。
消費者が居ない型を足してもテストで意味を固定できず、HTTP のエラー表現は PR10 が設計する。
PR10 は拒否理由を自前で再導出せず、必要ならこの判別子を足してから使うこと（持ち越し）。
