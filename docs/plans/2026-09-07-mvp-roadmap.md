...
- 並行できる組：`PR2〜PR4` と `PR5`、`PR8` と `PR6`、`PR13a` と `PR11`〜`PR12c`（2026-09-09 の見直しで `PR11` と `PR12` の並行は取り下げ。`PR11 → PR11c → PR11b → PR12a → PR12b → PR12c` の順。
  `PR11c` は web、`PR11b` は server なので互いに独立で、順序は入れ替えてよい。`PR12c` も server の小さな改善で
  `PR13a` とは独立に進められるため、`PR13a` との並行に含めている）。
...
- PR12a からの持ち越し：本文と右側（一覧・絞り込み・詳細）が独立してスクロールしない CSS を、
  このタスクの CSS 作業でまとめて拾う（`docs/plans/2026-09-11-pr12a-result-view.md` 持ち越し節）
- テスト：SSE の切断・再接続で状態が DB と一致すること、各状態の表示、操作の拒否理由の表示
- 受け入れ条件：11 節 3・5・14 項の UI 側
- 担当：Claude が画面設計、実装はサブエージェント（Claude）、Claude がレビューと検証
- 大きさ：中

### PR12c server：指摘一覧の 3N+1 解消
...
- 作る：`GET /api/runs/:id/findings` が指摘 1 件ごとに出している理由（`listReasons`）・再確認・採否の
  問い合わせをまとめて解消する。着手時に方式を設計し、再計測して固める
- 根拠：`listFindings` の理由に加えハンドラー側の再確認・採否の問い合わせも指摘 1 件ごとに出ており
  実質 **3N+1**。指摘 800 件（理由・再確認・採否つき）での計測で中央値 110〜130 ms 台（WSL2/Linux）
  だった。計測コードは `packages/server/src/api/findings.perf.test.ts`
