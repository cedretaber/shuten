...
interface LlmFinding { paragraphId: number; quote: string; before: string; after: string; suggestion: string | null; reason: string; verdict: InitialVerdict }
interface LlmCheckOutput { findings: LlmFinding[] }
type RecheckVerdict = "keep" | "withdraw" | "confirm-with-author"
interface LlmRecheckOutput { verdict: RecheckVerdict; reason: string }
...
function locateQuote(text: string, input: CheckInput, paragraphs: Paragraph[], finding: LlmFinding): LocateResult

// merge/merge.ts, merge/allowed-words.ts
interface Candidate { id: string; perspective: Perspective; llm: LlmFinding; locate: LocateResult }
interface MergedFinding { id: string; range: Range; quote: string; suggestion: string | null; sources: Candidate[]; verdict: InitialVerdict }
function mergeCandidates(candidates: Candidate[]): MergedFinding[]        // 同一実行内の呼び出し元が保証
...
| マイグレーション | `drizzle-kit generate` で SQL を生成しコミット。サーバー起動時に `migrate()` で適用 |
...
| エクスポート形式 | 実行 1 件を JSON 1 ファイルに（原稿版、設定、指摘、診断、採否を含む）。仕様書 8.2 節の「実装設計時に決める」に対応 |
...
```
PR1 → PR2 → PR3 → PR4 ─┐
PR5 → PR6 ─────────────┼→ PR7（PR1〜PR6 が必要）
PR1 → PR8 ─────────────┼→ PR9（PR2〜PR6、PR8）→ PR10 → PR11, PR12 → PR13
```
...
- 作る：`locate/locate.ts`、`locate/diagnostic.ts`、`locate/position-map.ts`（正規化後の文字列から原文への位置対応）
- 提供：`LocateResult`、`LocateFailureReason`、`Diagnostic`、`locateQuote`
- 規則：
  - 完全一致を `inputRange` 内で探す。1 件なら確定。複数なら段落 ID と `before` / `after` で絞る。絞れなければ `ambiguous`
  - 開始位置が `target.range` 外なら `outside-target`（診断記録に残す。付け替えない）
...
- 受け入れ条件：11 節 6・7・9・16 項の shared 側
- 担当：Claude がスペックと位置対応の設計、qwen が実装、Claude が検証
- 大きさ：中〜大
...
- 提供：`LlmFinding`、`LlmCheckOutput`、`LlmRecheckOutput`、`llmCheckOutputSchema`、`checkOutputJsonSchema`、`Candidate`、`MergedFinding`、`mergeCandidates`、`findSuppression`
- 規則：
  - 統合は同じ範囲・原文・修正案。修正案なしや別の問題は統合しない。観点と元候補への参照を保持
  - 抑制は「位置確定済みで、引用内の登録語の出現 1 箇所を空でない別の文字列に置き換えるだけで修正案を再現できる」。外側は完全一致。削除、複数出現にまたがる変更、範囲外に及ぶ変更は抑制しない
  - 抑制結果に登録語と規則版を含める
- テスト：リュシア→ルシア（抑制）、登録語だけの引用（抑制しない）、変更の一部に登録語（抑制しない）、複数出現のうち 1 箇所（抑制）、削除（抑制しない）、修正案 null、`finish_reason` 以外の理由での不正 JSON をスキーマが拒否
- 受け入れ条件：11 節 10・11 項の shared 側、18 項の一部
- 担当：Claude がスペック、qwen が実装、Claude が検証
- 大きさ：中
...
- 提供：`buildCheckRequest(input: CheckInput, text, perspective, allowedWords, settings): ChatRequest`、`buildRecheckRequest(...)`、`parseCheckResponse(result: ChatResult): LlmCheckOutput`（PR4 のスキーマで検証）
...
- 仕様：10（比較実験）、13（プロンプトと生成設定の実測）
- 作る：`packages/cli/`（新規パッケージ。`bin/shuten-eval.ts`）
- 機能：原稿ファイルと設定を受け取り、分割 → 観点別検査 → 位置確定 → 統合・抑制 → 再確認（任意）を DB なしで実行し、結果を JSON に出す。`--mode full-text` で全文を 1 要求で送る比較用の経路。実行条件（モデル、量子化、コンテキスト長、seed、思考の有無、プロンプト版）を JSON に含める
- 評価指標の集計（検出率、誤検出、位置特定失敗率）は正解ファイルとの突き合わせで PR13 に回す
- テスト：結合テストは統合テストとして分離。単体は引数解釈と JSON 形式
- 担当：Claude が設計、qwen が実装、Claude が実原稿での試運転
- 大きさ：中
...
- 規則：8.1 の表の項目をすべて持つ。原稿版は本文と本文ハッシュ。指摘は元候補への参照、位置特定状態、未確定位置を許す。採否は指摘 ID ごとに 1 件で更新日時を持つ。API キーは保存しない
...
- 作る：`server/src/run/queue.ts`（同時実行 1）、`run/orchestrator.ts`、`run/state.ts`（状態遷移）、`run/recovery.ts`（生成終了の確認と上限付き待機）
- 規則：
...
- 受け入れ条件：11 節 3・4・5・11・14・15・17 項
...
- 規則：本文は保存済み範囲で強調し、ブラウザで再計算しない。同じ範囲の複数指摘を参照可能。抑制候補・撤回候補は既定で非表示、切り替え可。「採用予定」は本文を書き換えない旨を表示。診断候補は通常の位置と区別
- テスト：UTF-16 範囲から DOM の強調への変換（絵文字・結合文字を含む本文で位置がずれない）、絞り込み、採否の更新
- 受け入れ条件：11 節 6・7・9・10・11・12・13 項の UI 側
- 担当：Claude が画面設計と位置→DOM 変換の設計、qwen が実装、Claude が検証
- 大きさ：大
...
| 3 | 全本文が検査対象に割り当てられ、観点ごとの処理状態を確認できる | PR2、PR9、PR12 |
| 4 | 参考文脈付きの分割検査と、文脈を広げた再確認が動作する | PR2、PR6、PR9 |
