...
```
PR1 → PR2 → PR3 → PR4 ──┬→ PR6 ──┐
PR5 ────────────────────┘        ├→ PR7（PR1〜PR6）→ PR9a（PR7、PR8）→ PR9b → PR10 → PR11 → [Windows] → PR11c → PR11b → [スパイク] → PR12a → PR12b → PR13b
...
- 並行できる組：`PR2〜PR4` と `PR5`、`PR8` と `PR6`、`PR13a` と `PR11`〜`PR12b`（2026-09-09 の見直しで `PR11` と `PR12` の並行は取り下げ。`PR11 → PR11c → PR11b → PR12a → PR12b` の順。
  `PR11c` は web、`PR11b` は server なので互いに独立で、順序は入れ替えてよい）。
