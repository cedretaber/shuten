...
PR13a-3 cli         全文チャット方式（自由形式プロンプト）の CLI モード（完了）
...
### PR13a-3 cli：全文チャット方式（完了）

- 仕様：10 全体（「現在の全文チャット方式」との比較）
- 詳細計画：`docs/plans/2026-09-12-pr13a-evaluation-export.md`（決定 15・34〜40、Task 13〜16）
- 作った：`full-chat` サブコマンド（`--manuscript` / `--model` / `--prompt-file` が必須）、
  ユーザーのプロンプトファイルへの `{{manuscript}}` の差し込み、`FullChatResult`
  （`formatVersion: "full-chat/1"`）、`isGenerationCapable` による種別違いのモデルへの送信拒否、
  `evaluate` / `aggregate` が全文チャットの結果を受け取らないようにする前置き検査
- 規則：構造化出力を使わず system も付けない（`user` 1 通だけ）。生成は 1 回だけで**再試行しない**。
  `finish_reason` が `stop` 以外（`length`・`tool_calls` など）はすべて失敗として記録し、打ち切られた
  本文を成功として保存しない（決定 37）。結果 JSON に原稿本文もプロンプトも入れず、`promptHash`
  （差し込み前のプロンプトのハッシュ）だけを残す（決定 36）。**自動採点しない**（決定 15・39）
- `packages/server` には何も足していない（決定 34。分割も検査単位も持たない実行を `PipelineResult` と
  `RunRecord` に混ぜないため）
- テスト：T12・T21・T29〜T34。指定した変異でそれぞれ落ちることを確認済み
- 受け入れ条件：本 PR だけでは満たすものなし（比較の道具を作っただけで、比較そのものは PR13b）
- 前提：PR13a-1 の後（CLI のサブコマンド化が要る）。**ユーザーの決定により PR13a-2 の後・PR13b の前に実施した**
- 担当：Claude が設計、実装はサブエージェント（Claude）、Claude がレビューと検証
- 大きさ：小
...
- 確定：モデル、思考の有無、生成パラメーター、分割長・文脈長、タイムアウト、`recoveryConfirmMs`、入力上限（仕様書 13 節。
  ユーザーが決め、仕様書と決定記録に記す）
- 受け入れ条件：11 節 20・21 項
- 前提 1：**PR13a-3（済。#27）**。2026-09-12 のユーザーの決定（詳細計画 Q4）で PR13b の前に実施した。
  これにより仕様書 10 節が求める比較を分割せず、`split` / `split-recheck` / `full-chat`（自由形式）の
  3 方式に `full-text`（構造化出力の対照）を加えた 4 方式を 1 巡で扱う。
  `full-chat` の結果は自動採点できない（PR13a 決定 15）ので、その指標だけは人が数える。
  非決定性は他の方式と同じなので `full-chat` も 3 回実行してぶれを記録する（詳細計画 決定 5c）
- 前提 2：**`check-truth` サブコマンド（未着手。詳細計画 Task 0・決定 17）**。正解ファイルだけを
  検証する手段が今の CLI に無く（`hash` は原稿しか見ず、`evaluate` は結果 JSON が必須）、
  このままでは正解ファイルの誤りに気付くのが最初の LLM 実行の後になる。ユーザーが正解ファイルを
  書き始める前に、独立した小さい PR として出す
- 評価原稿は 2 本（同上 Q1）。段階 A〜E を 1 本目、段階 F を 2 本目で行う。過剰適合の比較は
  **同じ経路・同じ本数**（どちらも CLI・確定条件で 3 回の集計）で行い、サーバー経由の通し実行は
  経路も本数も違うので比較には使わず通し動作の確認として残す（詳細計画 決定 7b・8）
- 担当：Claude が集計と記録、評価原稿と正解と最終判断はユーザー
- 大きさ：中（実施の時間は別）
- 思考ありは**観察のみ**（詳細計画 決定 5d）。`medium` を確定条件にすると、その後の 12 実行
  （段階 E 6・段階 F 3・サーバー通し 2・命令文原稿 1）がすべて思考ありになり 6〜18 時間かかるため、
  予算内では確定候補にしない。明らかに良かったときは測定を止めてユーザーに追加予算の判断を仰ぐ
- 詳細計画：`docs/plans/2026-09-12-pr13b-real-manuscript-evaluation.md`（決定 1〜17、Task 0〜6、Q1〜Q4）
