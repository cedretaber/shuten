...
- 確定：モデル、思考の有無、生成パラメーター、分割長・文脈長、タイムアウト、`recoveryConfirmMs`、入力上限（仕様書 13 節。
  ユーザーが決め、仕様書と決定記録に記す）
- 受け入れ条件：11 節 20・21 項
- **全文チャット方式の比較は第 1 巡では行わない**（詳細計画 決定 2）。PR13a-3（自由形式プロンプト）が
  ユーザーの判断で保留中のため、第 1 巡は `split` / `split-recheck` / `full-text`（構造化出力を全文に
  1 回かける対照）の 3 方式で比較し、第 1 巡の結果を見て PR13a-3 の要否を決める。必要になったら
  第 2 巡として自由形式全文チャットとの比較を評価記録に追記する。仕様書 10 節が求める比較を
  一部先送りすることになるので、ここに明記する
