...
- 確定：モデル、思考の有無、生成パラメーター、分割長・文脈長、タイムアウト、`recoveryConfirmMs`、入力上限（仕様書 13 節。
  ユーザーが決め、仕様書と決定記録に記す）
- 受け入れ条件：11 節 20・21 項
- 前提：**PR13a-3 を先に実施する**（2026-09-12 のユーザーの決定。詳細計画 Q4）。これにより
  仕様書 10 節が求める比較を分割せず、`split` / `split-recheck` / `full-chat`（自由形式）の
...
- 大きさ：中（実施の時間は別）
- 詳細計画：`docs/plans/2026-09-12-pr13b-real-manuscript-evaluation.md`（決定 1〜16、Task 1〜6、Q1〜Q4）
