...
| エクスポート形式 | 実行 1 件を JSON 1 ファイルに（原稿版、設定、指摘、診断、採否を含む）。先頭に形式の版 `formatVersion` を持つ。接続先 URL は含めない。仕様書 8.2 節の「実装設計時に決める」に対応（PR13a） |
...
PR13a server+cli  評価ツール（正解との突き合わせと集計、全文チャット方式のプロンプト）、エクスポートの口と形式
PR13b all    実原稿での評価の実施と、仕様書 13 節の確定
```

チェックポイント（PR ではない作業）：

- **Windows でのローカル確認**：PR10 のマージ後・PR11 の前（`docs/guides/windows-verification.md`）。
- **強調表示のスパイク**：PR11 の後・PR12a の設計前。UTF-16 範囲 → DOM の方式を確かめる。コードは残さない。
- **評価原稿と正解データの準備**：今から。PR13a の設計で形式を決め、13b までに揃える（ユーザー）。

依存関係：

```
PR1 → PR2 → PR3 → PR4 ──┬→ PR6 ──┐
PR5 ────────────────────┘        ├→ PR7（PR1〜PR6）→ PR9a（PR7、PR8）→ PR9b → PR10 → [Windows] → PR11 → PR11b → [スパイク] → PR12a → PR12b → PR13b
PR1 → PR8 ───────────────────────┘                                                    └→ PR13a（PR10 の後いつでも。PR13b の前）
```

（2026-09-09 の見直し後。見直しの内容は「2026-09-09 の見直し」節）
...
- 並行できる組：`PR2〜PR4` と `PR5`、`PR8` と `PR6`、`PR13a` と `PR11`〜`PR12b`（2026-09-09 の見直しで `PR11` と `PR12` の並行は取り下げ。`PR11 → PR11b → PR12a → PR12b` の順）。
...
- Windows でのローカル確認を PR5 以降一度もしていない（CI のみ）。PR10 のマージ後・PR11 の前にチェックポイントを置く。
- PR10 の接続確認から試験生成を外した（生成終了未確認を表現できないため。PR10 計画の決定 8）。受け入れ条件 1 項は
  PR11 の最初の検査実行で満たす。エクスポートは PR10 から PR13a へ移した（PR10 計画の決定 16）。どちらも PR10 の実施で確定した。
...
- 仕様 10 節の「全文チャット方式との比較」は残す。自由形式プロンプトと CLI モードを PR13a で足す
  （現在の `full-text` モードは分割の対照にすぎない）。
- `listFindings` の N+1 は PR12a で件数を見て判断する（持ち越しのまま）。
...
| **評価原稿と正解データ**（仕様書 10 節。作者が利用を認めた原稿に実在した誤りの一覧と、誤検出してほしくない正常な文章） | **今から準備を始める。** 形式は PR13a の設計で決め、PR13b の実施までに揃える |
| Windows でのローカル確認（PR10 マージ後） | PR11 の前のチェックポイント |
...
- 前提：Windows でのローカル確認（チェックポイント）を済ませてから着手する
- 仕様：4（手順 1〜4）、5.1、5.2
...
### PR11b server：接続断で失敗した単位の復旧を 1 段にする

- 2026-09-09 の見直しで追加。PR9b からの持ち越し「生成中に応答を受け取れずに接続が切れた検査単位は `failed` のまま残る」の解消
- 仕様：8.2（失敗した処理の個別再試行、復旧待ち）
- 作る：`UnitFailure` / `UnitFailureRecord` に「応答を受け取ったか」を持たせ、応答を受け取らずに切れた単位は実行を
  `recovery-waiting` にするのと同じトランザクションで `pending` に戻す。PR9 決定 20（打ち切りの経路）の改訂を同じ PR で行う
...
- 作る：`features/results/`（本文表示、強調、指摘一覧、詳細、採否、絞り込み）。完了済み・停止中の実行を `GET /api/runs/:id`・
  `findings`・`findings/:id` から読んで表示する。実行一覧（`GET /api/runs`）からの再閲覧
...
- 作る：開始 → 進捗 → 完了の流れ、停止・再開・失敗単位の再試行・復旧確認の操作、SSE の購読と `run-settled` 後の再購読、
  再読み込み・再接続時の `GET /api/runs/:id` による復元、`recovery-waiting` / `recovery-blocked` / `backend-restarted` /
  再開拒否（`run-rejected-*`）の案内
- 規則：進捗は完了した検査単位数と再確認件数（`progress`）で示す。「完了」「一部失敗」「停止中」「実行中」「復旧待ち」を区別し、
  未処理範囲がある状態を「問題なし」と表示しない。再開・新規開始を区別する
- テスト：SSE の切断・再接続で状態が DB と一致すること、各状態の表示、操作の拒否理由の表示
- 受け入れ条件：11 節 3・5・14 項の UI 側
- 担当：Claude が画面設計、実装はサブエージェント（Claude）、Claude がレビューと検証
- 大きさ：中

### PR13a server + cli：評価ツールとエクスポート
...
- 仕様：10 全体、8.2（エクスポート）
- 作る：正解ファイルの形式（設計で決め、ユーザーの準備に渡す）、`packages/cli/` に正解との突き合わせと集計（検出率、誤検出、
  位置特定失敗率、診断候補の正誤、抑制の適否、所要時間）、複数回実行の集計、全文チャット方式の自由形式プロンプトと CLI モード、
  エクスポートの口
- 前提：PR10 の後ならいつでも着手できる。PR13b の前に終える
