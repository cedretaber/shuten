...
**規則に例外を 1 つ足した（PR #18 のレビュー指摘）。** 確定した 4xx のうち、`code` が
`invalid-run-settings` のものだけは、送信後に決まった失敗でも `"settings"` にする。
`packages/server/src/api/runs.ts` の `validateHardTimeouts()` は、画面が `checkMs` を個別の上限内に
収めていても、サーバーのプロセス設定 `recoveryConfirmMs` を足した**合計**が `MAX_TIMEOUT_MS` を
超えると 400 で拒む。画面はそのプロセス設定を知らないので、この検証はクライアントでは完結できない。
まさに「画面上に無い設定値についてのエラー」であり、この決定が拾おうとしている状況そのものである。
「送信前か送信後か」という切り分けは、この 1 つを取りこぼしていた。判定は `status` ではなく
`code` で行う（同じ 400 でも `validation` など別の意味の code がある）。
...
- S6-5：確定した 4xx でも `code` が `invalid-run-settings` なら `hint` が `"settings"`。
  同じ 400 でも別の `code` なら `"none"` のまま（これを入れないと「4xx を全部 `"settings"` にする」
  実装でも通ってしまう）。
