...
5. **絞り込みは 段落 ID → before → after の順に、残りが 1 件になるまで適用する。有効なヒントが全候補と矛盾したら
   そこで打ち切り、残っている候補で失敗にする（厳密適用）。** ヒントが無い（段落 ID が入力範囲と重なる段落に存在しない、
   before / after が改行を除いて空）フィルタは飛ばす（表 A3、A7、K1）。段落 ID が本文の他の段落に存在しても、
   入力範囲と重ならなければ「存在しない」と同じに扱う。完全一致はすべて入力範囲内にあるので、入力範囲外の段落を
   指すヒントは必ず全候補と矛盾し、モデルの誤りとしか解釈できないためである（表 K1〜K3）。1 件になった時点で残りのフィルタは見ない
...
- **ヒントの厳密適用（決定 5）。** 仕様 6.2 の「前後の引用の不備だけを理由に失敗にしない」は一意な場合の規定で、
  ...
  打ち切る厳密適用に改めた。入力範囲と重なる段落に無い段落 ID はヒントなしとして扱う（モデルが段落 ID を誤ることは
  多く、それだけで失敗にすると失敗が増えすぎる）。初版は本文全体で存在を判定していたため、本文にはあるが入力範囲外の
  段落 ID が有効なヒントとして全候補と矛盾し、before / after を見ずに打ち切っていた。後続修正（PR #8）で入力範囲との
  重なりを条件に改めた。ヒントの軽微な崩れで失敗が増える不便は、モデル出力の観察で見直す。
...
- `locate`（表 A〜H、J、K）：段落 ID で絞る、段落 0 を指して `outside-target`、段落 ID 不在で `after` で絞る、
  ...
  候補の無い段落を挟んだ同順位（表 J1）、本文にはあるが入力範囲外の段落 ID を飛ばして `after` で絞る（表 K1）、
  入力範囲内で一致の無い段落 ID は打ち切り（表 K2）、入力範囲と部分的に重なる段落 ID は有効（表 K3）、
