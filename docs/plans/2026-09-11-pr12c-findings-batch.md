...
**Task 4 の再計測結果**（`packages/server/src/api/findings.perf.test.ts`。WSL2 上の Linux、
指摘 800 件・各件に理由 1・再確認 1・採否 1 を添えた状態、Windows 未確認）：

| 実行 | サンプル（ms） | 中央値（ms） |
| --- | --- | --- |
| 1 回目 | 12.56, 11.58, 12.10, 11.92, 9.81 | 11.92 |
| 2 回目 | 12.89, 13.22, 11.87, 12.34, 10.36 | 12.34 |
| 3 回目 | 14.16, 13.08, 12.67, 12.65, 10.86 | 12.67 |

改善前（本ブランチ着手時点の再現値）は `median=125.12ms`（決定 14 の記述と同じく WSL2/Linux で計測）。
3 回とも中央値は 12 ms 前後で、改善前の約 1/10 に縮み、決定 14 の判断基準だった 100 ms は
もちろん、テストに残した上限 5,000 ms も大きく下回った。**索引を足さない判断は妥当だった**と
数字で確認できた——バッチ化後に残る全表走査（`findings` 1 本・`recheck_units` 1 本・
`judgments` × `findings` の結合 1 本）は、指摘 800 件の規模でも 12 ms 前後にしかならず、
`candidates.finding_id` への索引（対象外にした選択肢の 1 つ）を足す動機は数字の上でも無い。
MVP の規模を超えて指摘が万単位になるなど前提が変わらない限り、見直しは不要と判断する。
...
- 索引の追加による更なる高速化は行わない（決定 5）。再計測の値が期待どおりでなければ、
  ...
  → **Task 4 で再計測、期待どおり（中央値 12 ms 前後で 100 ms・5,000 ms のいずれも大きく下回った）。
  索引を足さない判断は妥当と確認できたので見直さない（決定 5 に追記）**。
- `findings.query-count.test.ts` の `seedRun` / `seedManyFindings` が `findings.perf.test.ts` と
  ほぼ同じ形で重複している（Task 3 レビュー Minor 4）。共通化すると性能テストの計測条件が静かに
  変わる危険の方が大きいので、本 PR では共通化しない（コーディネーターの裁定）。
