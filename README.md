...
開始ができる。原稿と検査設定の画面、設定の画面、検査実行の受付表示の 3 画面と、それらをつなぐアプリの骨格・API
クライアントを持つ。
表示は開始直後の受付（実行 ID と状態）と、保存済みの結果閲覧（PR12a）まで。進捗表示・停止再開などの
実行制御は PR12b で作る。
...
（`docs/experiments/2026-09-11-highlight-spike/README.md`）。続けて**PR12a（結果閲覧）まで完了**し、
保存済みの実行を開いて本文・強調・指摘一覧・詳細を読み、採否を記録できるようになった。実行一覧
（`/runs`）から過去の実行を再閲覧できる。開始・進捗・停止・再開などの実行制御と SSE は次の PR12b で作る。
`GET /api/runs/:id/findings` の問い合わせ回数の見直し（実質 3N+1）は、計測して 100 ms を上回ったため
据え置きをやめ PR12c に回した（ロードマップの PR12a・PR12c 節、詳細計画は
`docs/plans/2026-09-11-pr12a-result-view.md`）。2026-09-09 に残りの工程を見直し、
