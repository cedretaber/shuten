...
受け入れ条件のうち 11 節 3・4・5・11・14・15・16・18 項をサーバー側で満たした。**PR11（接続・入力・設定画面）
まで完了**し、ブラウザの画面から接続設定（LM Studio の接続先 URL・API キーの設定、モデルの選択）、
原稿の確定（貼り付け／ファイル読み込み）、検査設定の入力と検査の開始ができる。原稿と検査設定の画面、
接続設定の画面、検査実行の受付表示の 3 画面と、それらをつなぐアプリの骨格・API クライアントを持つ。
表示は開始直後の受付（実行 ID と状態）までで、進捗・結果一覧・停止再開などは PR12a・PR12b で作る。
...
（画面とクライアント）。次は Windows でのローカル確認（チェックポイント。マージ条件）、続いて PR11b
（接続断で失敗した単位の復旧）と PR12a・PR12b（結果閲覧・実行制御）。2026-09-09 に残りの工程を見直し、
