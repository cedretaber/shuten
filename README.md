...
実装はロードマップ（`docs/plans/2026-09-07-mvp-roadmap.md`）の PR 単位で進めており、PR8（DB スキーマと永続化）まで完了。
PR9 は基盤とオーケストレーターの 2 本（PR9a・PR9b）に分けて進めており、PR9a（検査単位・再確認単位の実行、
単一キュー、状態遷移、境界検証、増分マージの各部品）まで完了。これらはまだ実行の開始・停止・再開を
つなぐオーケストレーターに組み込まれていないため、利用者から見える動きはまだ増えていない。次は PR9b
（オーケストレーター本体）。詳細計画は `docs/plans/2026-09-09-pr9-orchestration.md`。
